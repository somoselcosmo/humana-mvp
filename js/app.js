...
async function ingresarAlDashboard(usuario) {
  ...
  if (usuario.rol === "pendiente") {
    ...
    setText(
      "user-role-display",
      usuario.rol ? usuario.rol.toUpperCase() : "VECINO"
    );
...
if (btnToggleLibros) {
  btnToggleLibros.addEventListener("click", () => {
    ...
    iconArrowLibros.style.transform = isHidden
      ? "rotate(180deg)"
      : "rotate(0deg)";
  });
}
...
// --- B. CARGAR DIRECTIVA (CORREGIDO: TODOS LOS ÓRGANOS) ---
async function cargarDirectiva(jacId) {
  const gridContainer = document.getElementById("grid-gobierno");
  if (!gridContainer) return;
  gridContainer.innerHTML =
    '<div style="color:white; padding:20px;">Cargando estructura...</div>';
  ...
  try {
    ...
    if (snapshot.empty) {
      gridContainer.innerHTML =
        '<div style="color:#aaa;">No hay directiva visible.</div>';
      return;
    }

    // 1. CLASIFICAR (Ahora incluimos Fiscal y Convivencia)
    let dignatarios = [];
    let comites = [];
    let fiscalia = [];
    let convivencia = [];
    ...
    snapshot.forEach((doc) => {
      ...
      else if (data.grupo === "fiscalia") fiscalia.push(data);
      else if (data.grupo === "convivencia") convivencia.push(data);
    });

    gridContainer.innerHTML = "";

    // 2. PINTAR DIGNATARIOS
    if (dignatarios.length > 0) {
      // A. Encontrar al Presidente
      const presidente =
        dignatarios.find(
          (d) => d.cargo && d.cargo.toLowerCase().includes("presidente")
        ) || dignatarios[0];

      // B. Encontrar al resto del equipo (Excluyendo al presidente)
      // (Usamos el ID o el email para filtrar al presidente y no repetirlo)
      const equipo = dignatarios.filter((d) => d.email !== presidente.email);

      // C. Generar HTML de la lista de los otros 3
      let htmlEquipo = '<div class="gov-mini-list">';

      // Orden sugerido: Vice, Tesorero, Secretario (Si existen)
      // Hacemos un sort simple o los mostramos como vengan
      equipo.forEach((miembro) => {
        htmlEquipo += `
                    <div class="gov-mini-item">
                        <div class="gov-mini-avatar">${getIniciales(
                          miembro.nombre
                        )}</div>
                        <div class="gov-mini-info">
      ...
      // D. Armar la Tarjeta Completa
      const htmlDirectiva = `
                <article class="gov-card">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                        <div class="gov-badge-top" style="margin:0;">JUNTA DIRECTIVA</div>
                        <span style="color:#aaa; font-size:0.75rem;">Vigencia 2024-2028</span>
                    </div>
                    
                    <!-- SECCIÓN PRESIDENTE (DESTACADO) -->
                    <div class="gov-president-section">
                        <h2 class="gov-role-title" style="font-size:1.1rem; color:var(--cyber-blue); margin-bottom:10px;">PRESIDENTE(A)</h2>
                        <div class="gov-profile" style="margin-bottom:0;">
                            <div class="gov-avatar">${getIniciales(
                              presidente.nombre
                            )}</div>
                            <div class="gov-info">
                                ...
                                <p style="font-size:0.85rem; opacity:0.8;">Representante Legal</p>
                            </div>
                        </div>
                    </div>

                    <!-- SECCIÓN EQUIPO (LISTA) -->
                    ${htmlEquipo}

                    <div class="gov-footer" style="margin-top:20px;">
                        <a href="#" style="color:var(--cyber-blue); font-size:0.85rem; text-decoration:none;">Ver funciones y perfiles →</a>
                    </div>
                </article>
            `;
      ...
      gridContainer.innerHTML =
        '<div style="color:#aaa; padding:10px;">No hay dignatarios registrados.</div>';
    }

    // 3. PINTAR FISCAL (NUEVO)
    fiscalia.forEach((fiscal) => {
      gridContainer.innerHTML += `
                <article class="gov-card">
                    <div class="gov-badge-top" style="background: #4a1d96; color: #d8b4fe;">ÓRGANO DE CONTROL</div>
                    <h2 class="gov-role-title">${fiscal.cargo || "Fiscal"}</h2>
                    <div class="gov-profile">
                        <div class="gov-avatar" style="background: linear-gradient(135deg, #6441a5, #4a1d96);">${getIniciales(
                          fiscal.nombre
                        )}</div>
                        <div class="gov-info">
                            <h3>${fiscal.nombre}</h3>
                            <p style="opacity:0.7; font-size:0.8rem">Vigilancia y Control</p>
                        </div>
                    </div>
                    <div class="gov-footer">
                        <span style="color:#aaa; font-size:0.85rem;">Independiente</span>
                    </div>
                </article>
            `;
    });

    // 4. PINTAR CONVIVENCIA (NUEVO)
    convivencia.forEach((conciliador) => {
      gridContainer.innerHTML += `
                <article class="gov-card">
                    <div class="gov-badge-top" style="background: #0f3d4a; color: #22d3ee;">CONVIVENCIA</div>
                    <h2 class="gov-role-title">${
                      conciliador.cargo || "Conciliador"
                    }</h2>
                    <div class="gov-profile">
                        <div class="gov-avatar" style="background: linear-gradient(135deg, #0891b2, #06b6d4);">${getIniciales(
                          conciliador.nombre
                        )}</div>
                        <div class="gov-info">
                            <h3>${conciliador.nombre}</h3>
                            <p style="opacity:0.7; font-size:0.8rem">Justicia Comunitaria</p>
                        </div>
                    </div>
                    <div class="gov-footer">
                        <button class="gov-action-btn"><i class="ri-scales-3-line"></i> Casos</button>
                    </div>
                </article>
            `;
    });

    // 5. PINTAR COMITÉS (Igual que antes)
    comites.forEach((comite) => {
      const nombreComite =
        comite.comision_interes ||
        comite.nombre_comision ||
        "Comité de Trabajo";
      gridContainer.innerHTML += `
                <article class="gov-card">
                    <div class="gov-badge-top" style="background:#232d36;">COMITÉ DE TRABAJO</div>
                    <h2 class="gov-role-title" style="font-size:1.4rem; margin-bottom:10px;">${
                      comite.nombre
                    }</h2>
                    <p style="color:#77c7ff; font-size:0.9rem; margin-bottom:20px;">${
                      comite.cargo || "Coordinador"
                    }</p>
...
async function cargarLibro(jacId, tipoLibro) {
  ...
  if (tipoLibro === "afiliados") {
    ...
    if (subtituloEl)
      subtituloEl.textContent = "Registro legal de socios (Ley 2166).";
    thead.innerHTML = `<tr><th>Afiliado</th><th>Documento</th><th>Comisión</th><th>Estado</th></tr>`;
    cargarSolicitudesPendientes(jacId);
  ...
  if (btnNuevo) {
    ...
    if (tipoLibro === "convivencia")
      btnNuevo.innerHTML = '<i class="ri-add-line"></i> Agregar norma';
  ...
  try {
    ...
    if (snapshot.empty) {
      if (tbody)
        tbody.innerHTML =
          '<tr><td colspan="6" style="text-align:center; padding:20px; color:#aaa;">No hay registros.</td></tr>';
      return;
    }
    ...
    snapshot.forEach((doc) => {
      ...
      if (tipoLibro === "actas") {
        ...
        tr.innerHTML = `
                    <td style="font-family:monospace; color:white;">${
                      data.numero || "-"
                    }</td>
                    <td>${data.fecha}</td>
                    ...
                    <td>${
                      data.decisiones
                        ? data.decisiones.substring(0, 25) + "..."
                        : "-"
                    }</td>
                    <td><span class="acta-badge ${badge}">${
          data.estado
        }</span></td>
                    <td><button class="btn-editar btn-ver-acta">Ver</button></td>
                `;
        tr.querySelector(".btn-ver-acta").addEventListener("click", () =>
          abrirVisorActa(data)
        );
        tbody.appendChild(tr);
        return; // Importante return para no duplicar
      } else if (tipoLibro === "afiliados") {
        const icono = data.uid_usuario
          ? '<i class="ri-wifi-line" style="color:#10b981"></i>'
          : '<i class="ri-wifi-off-line" style="color:#555"></i>';
        const estadoColor = data.estado === "Inactivo" ? "#ef4444" : "#10b981";
        const nombre = data.nombre || `${data.nombres} ${data.apellidos}`;
        fila = `<tr>
                    <td style="display:flex; gap:10px; align-items:center;">${icono} <div><b style="color:white">${nombre}</b><br><span style="font-size:0.8em; color:#777">${
          data.telefono || ""
        }</span></div></td>
                    <td><span style="color:#777; font-size:0.8em">${
                      data.tipo_documento || "CC"
                    }</span> ${data.documento}</td>
                    <td>${data.comision_trabajo || "Asamblea"}</td>
                    <td><span class="badge" style="color:${estadoColor}; border:1px solid ${estadoColor}40">${
          data.estado || "Activo"
        }</span></td>
                </tr>`;
      } else if (tipoLibro === "contable") {
        const esIng = data.tipo === "Ingreso";
        if (esIng) sumaIng += Number(data.valor);
        else sumaEgr += Number(data.valor);
        const color = esIng ? "#10b981" : "#f87171";
        fila = `<tr>
                    <td>${data.fecha}</td>
                    <td><b style="color:white">${data.concepto}</b></td>
                    <td style="color:${color}; text-transform:uppercase;">${
          data.tipo
        }</td>
                    <td style="color:white;">$${new Intl.NumberFormat().format(
                      data.valor
                    )}</td>
                    <td>${data.soporte || "-"}</td>
                </tr>`;
...
async function cargarMensajes(jacId) {
  ...
  try {
    ...
    if (snapshot.empty) {
      contenedor.innerHTML =
        '<div style="padding:20px; text-align:center; color:gray">Bandeja vacía.</div>';
      return;
    }
    ...
    lista.forEach((data) => {
      ...
      item.addEventListener("click", () =>
        verMensajeCompleto(data, data.id, item)
      );
...
async function verMensajeCompleto(data, msgId, itemHTML) {
  ...
  if (rolesDir.includes(currentUserData.rol) && data.estado === "no_leido") {
    try {
      ...
      if (itemHTML) {
        itemHTML.classList.remove("unread");
        itemHTML.classList.add("read");
      }
    } catch (e) {}
  }
}
...
async function enviarMensajeNuevo(asunto, dest, cuerpo) {
  try {
    const nombre =
      currentUserData.nombres || currentUserData.nombre || "Vecino";
...
async function enviarRespuestaAdmin() {
  ...
  try {
    ...
    // Refrescar vista
    document
      .getElementById("formulario-respuesta")
      .classList.add("hidden-view");
    document
      .getElementById("visualizar-respuesta")
...
function activarNotificaciones(jacId) {
  if (
    !currentUserData.rol.includes("admin") &&
    !["presidente", "secretario"].includes(currentUserData.rol)
  )
    return;
  const q = query(
    collection(db, "jacs", jacId, "mensajes"),
...
if (btnIrRegistro)
  btnIrRegistro.addEventListener("click", () => {
    ...
    viewRegister.classList.remove("hidden-view");
    cargarListaDeJACs();
  });
...
async function cargarListaDeJACs() {
  ...
  try {
    ...
    snap.forEach((d) => {
      selectJac.innerHTML += `<option value="${d.id}">${
        d.data().nombre
      }</option>`;
    });
  } catch (e) {}
...
const configLibros = {
  afiliados: [
    ...
    { label: "Teléfono", type: "number", id: "telefono" },
    {
      ...
      type: "select",
      id: "comision_trabajo",
      options: ["Asamblea", "Obras", "Salud", "Deportes", "Seguridad"],
    },
    {
      label: "Estado",
      type: "select",
  ...
  inventario: [
    ...
    { label: "Nombre Bien", type: "text", id: "item" },
    {
      ...
      type: "select",
      id: "categoria",
      options: ["Tecnología", "Mobiliario", "Equipo"],
    },
    {
      label: "Estado",
      type: "select",
      id: "estado_bien",
      options: ["Operativo", "Mantenimiento", "Baja"],
    },
...
if (btnNuevoRegistro) {
  btnNuevoRegistro.addEventListener("click", () => {
    const menuActivo = document.querySelector(
      "#submenu-libros .menu-item.active"
    );
    ...
    document.getElementById("modal-titulo-texto").textContent =
      "Nuevo: " + tipoLibro.toUpperCase();
    ...
    campos.forEach((c) => {
      ...
      if (c.type === "select") {
        ...
        inputHTML = `<input type="${c.type}" id="input-${
          c.id
        }" class="inbox-input" placeholder="${c.placeholder || ""}">`;
      }
      const div = document.createElement("div");
      if (
        ["direccion", "comision_trabajo", "descripcion", "concepto"].includes(
          c.id
        )
      )
...
if (formModal) {
  formModal.addEventListener("submit", async (e) => {
    ...
    try {
      await addDoc(
        collection(db, "jacs", currentJacId, "libro_" + libroActualParaGuardar),
        data
      );
      alert("Guardado.");
...
if (btnCancelGen)
  btnCancelGen.onclick = () => modalRegistro.classList.add("hidden-view");
if (btnCerrarModal)
  btnCerrarModal.onclick = () => modalRegistro.classList.add("hidden-view");
...
async function abrirModalActas() {
  ...
  try {
    ...
    selPresi.innerHTML = def;
    selSecre.innerHTML = def;
...
if (btnGuardarActa) {
  btnGuardarActa.addEventListener("click", async () => {
    ...
    document.querySelectorAll(".input-orden").forEach((i) => {
      if (i.value) nuevaActa.orden_dia.push(i.value);
    });
    ...
    try {
      await addDoc(
        collection(db, "jacs", currentJacId, "libro_actas"),
        nuevaActa
      );
      alert("Acta guardada.");
      ...
      alert("Error guardar acta");
    }
  });
}
if (btnCerrarActa)
  btnCerrarActa.onclick = () => modalActa.classList.add("hidden-view");
if (btnCancelarActa)
  btnCancelarActa.onclick = () => modalActa.classList.add("hidden-view");
...
function abrirVisorActa(data) {
  ...
  lst.innerHTML = "";
  if (data.orden_dia)
    data.orden_dia.forEach((i) => (lst.innerHTML += `<li>${i}</li>`));
...
// =========================================================
// 11. LÓGICA DE APROBACIÓN DE USUARIOS
// =========================================================

// A. CARGAR Y PINTAR SOLICITUDES
async function cargarSolicitudesPendientes(jacId) {
  // Solo Presidente y Secretario pueden ver esto
  if (!["presidente", "secretario", "admin"].includes(currentUserData.rol))
    return;

  // Buscamos o creamos el contenedor dentro de la sección de libros
  let container = document.getElementById("requests-area");
  if (!container) {
    // Inyectamos el contenedor ANTES de la tabla de libros
    const tablaSection = document.querySelector(
      "#section-libros .council-section"
    );
    container = document.createElement("div");
    container.id = "requests-area";
    tablaSection.parentNode.insertBefore(container, tablaSection);
  }

  try {
    const usuariosRef = collection(db, "usuarios");
    // Buscamos usuarios pendientes de ESTA JAC
    const q = query(
      usuariosRef,
      where("jacId", "==", jacId),
      where("rol", "==", "pendiente")
    ...
    if (snapshot.empty) {
      container.innerHTML = ""; // Si no hay nada, limpiamos
      return;
    }
    ...
    snapshot.forEach((doc) => {
      ...
      htmlSolicitudes += `
                <div class="request-card">
                    <div class="req-info">
                        <b>${data.nombre}</b>
                        <span>CC. ${data.documento} • ${
        data.direccion || "Sin dirección"
      }</span>
                    </div>
                    <button class="btn-approve" onclick="aprobarUsuario('${
                      doc.id
                    }', '${data.documento}')">
                        <i class="ri-check-line"></i> Aprobar Acceso
                    </button>
                </div>
            `;
    });
    ...
    container.innerHTML = `
            <div class="requests-container">
                <div class="requests-header">
                    <h3><i class="ri-user-add-line"></i> Solicitudes de Afiliación (${snapshot.size})</h3>
                </div>
                ${htmlSolicitudes}
            </div>
        `;
...
// B. FUNCIÓN DE APROBAR (Global para poder llamarla desde el HTML)
window.aprobarUsuario = async function (uidUsuario, documento) {
  if (
    !confirm(
      "¿Confirmas que este usuario es un vecino real y apruebas su acceso?"
    )
  )
    return;

  try {
    // 1. Actualizar Rol en Colección Usuarios (Permite el login)
    const userRef = doc(db, "usuarios", uidUsuario);
    await updateDoc(userRef, { rol: "vecino" });

    // 2. Actualizar Estado en Libro de Afiliados (Sincronización)
    // Buscamos en el libro por documento para actualizar su ficha legal
    const libroRef = collection(db, "jacs", currentJacId, "libro_afiliados");
    const q = query(libroRef, where("documento", "==", documento));
    const snapshot = await getDocs(q);

    if (!snapshot.empty) {
      const fichaId = snapshot.docs[0].id;
      await updateDoc(
        doc(db, "jacs", currentJacId, "libro_afiliados", fichaId),
        {
          estado: "Activo",
          uid_usuario: uidUsuario, // Aseguramos el vínculo
        }
      );
    }
    ...
    // Recargar la vista
    cargarLibro(currentJacId, "afiliados");
  } catch (error) {
